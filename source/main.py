...
class Preprocesor:
    ...
    def train_with_neural_network(self):
        self.prepare_data_fro_training()
        x_train, x_test, y_train, y_test = self.split_train_test()
        model = Sequential()
        ...
        # Fit the model
        history = model.fit(x_train, y_train, validation_data=(x_test, y_test), epochs=10, batch_size=70) #, verbose=0
